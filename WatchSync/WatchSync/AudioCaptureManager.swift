...
final class AudioCaptureManager: ObservableObject {
    ...
    /// URL of the most recently finished raw recording (for diagnostics export).
    ...
    func start(prepare: @escaping (Double) -> Void, onStarted: @escaping () -> Void) {
        engineQueue.async { [weak self] in
            ...
            // Open a raw WAV recording alongside the live analysis so the exact
            // audio can be replayed and the DSP tuned offline. Written to
            // Documents (UIFileSharingEnabled) so it survives and can be pulled
            // off the device via Finder / `xcrun devicectl`.
            let ts = Int(Date().timeIntervalSince1970)
            let docs = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0]
            let url = docs.appendingPathComponent("watch-\(ts).wav")
            do {
                self.recordFile = try AVAudioFile(forWriting: url, settings: format.settings)
                self.recordURL = url
            } catch {
